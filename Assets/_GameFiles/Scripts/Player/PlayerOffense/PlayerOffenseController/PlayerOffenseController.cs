...
public sealed class PlayerOffenseController
{
    ...
    private readonly PlayerAttackCloseRange _attackCloseRange = new PlayerAttackCloseRange(); //тут надо зависеть от абстракции, хот€ мы хотим чтобы были именно ближн€€ и дальн€€ атака, Ќќ лучше сделать зависимость от абстракции
    ...
    public PlayerOffenseController(Vector3 position, Vector2 direction, WeaponCloseRange weaponCloseRange, WeaponLongRange weaponLongRange)
    {
        ...
        _attackCloseRange.Initialize(new EnvironmentAreaOverlapAnalyzer<IDamageable, PlayerController>(), position, direction, 0.15f, weaponCloseRange.Range, 1.25f, new DamageCalculatorBasic()); //работа с конретной реализацией должна проводитьс€ наверху, но с другой стороны мы создаем крепкий контрактна то, что в €чейку ближней атаки не заинититс€ дальн€€ атака (что по идее может прилететь с сервера спокойно)
        _attackLongRange.Initialize(new EnvironmentAreaOverlapAnalyzer<IDamageable, PlayerController>(), position, direction, 0.15f, weaponLongRange.Range, 1.25f, new DamageCalculatorBasic()); //работа с конретной реализацией должна проводитьс€ наверху, но с другой стороны мы создаем крепкий контрактна то, что в €чейку ближней атаки не заинититс€ дальн€€ атака (что по идее может прилететь с сервера спокойно)
    }
